...
use mpl_core::{
instructions::{TransferV1CpiBuilder, UpdatePluginV1CpiBuilder}, types::{FreezeDelegate, Plugin}
};
use sol_mind_protocol::{Operation, ProjectConfig, helpers::pay_protocol_fee};
...
impl<'info> Purchase<'info> {
    pub fn purchase_asset(&mut self) -> Result<()> {
        ...
        let protocol_fee = self.protocol_config.calculate_fee_amount(Operation::TradeNFT, Some(asset_price));
        ...
        let trade_hub_key = self.trade_hub.project.key();
        let seeds = &[
            b"trade_hub",
            self.trade_hub.name.as_bytes(),
            trade_hub_key.as_ref(),
            &[self.trade_hub.bump],
        ];
